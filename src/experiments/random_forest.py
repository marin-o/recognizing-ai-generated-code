...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a Random Forest classifier on CST features")
    parser.add_argument("--cache-dir", type=str, default="data/", help="Directory to cache dataset")
    parser.add_argument("--test-size", type=float, default=0.2, help="Proportion of dataset for test set")
    parser.add_argument("--val-size", type=float, default=0.1, help="Proportion of training set for validation")
    parser.add_argument("--tune-hyperparameters", action="store_true", default=True, help="Whether to perform hyperparameter tuning using GridSearchCV")
...
def main():
    ...
    dataset = AIGCodeSet_WithCSTFeatures(cache_dir=args.cache_dir, features_as_tensor=False)
    train, val, test = dataset.get_dataset(split=True, test_size=args.test_size, val_size=args.val_size)
    ...
    feature_columns = [
        "function_defs", "if_statements", "loops", "imports", "comments",
    ...
    # Extract features
    train_cst_features = np.array([train[col] for col in feature_columns]).T  # Shape: (n_samples, 9)
    ...
    logger.info(f"Class distribution (train): Human={class_counts[0]}, AI={class_counts[1]}")
    ...
    logger.info(classification_report(val_labels, val_predictions, target_names=["human", "ai"]))
    logger.info("Test Classification Report:")
    logger.info(classification_report(test_labels, test_predictions, target_names=["human", "ai"]))
