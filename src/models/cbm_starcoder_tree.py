...
class CBMStarCoderTree(nn.Module):
    ...
    def __init__(
        ...
        if model_name is not None:
            ...
            raise ValueError(f"Invalid backbone_type: {backbone_type}. Must be 'starcoder' or 'codebert'")
        ...
        if backbone_type == "codebert" and model_name is None:
            ...
            self.backbone = AutoModel.from_pretrained(self.model_name, trust_remote_code=True)
    ...
    def forward(self, inputs: Dict[str, torch.Tensor], codes: Optional[list] = None, languages: Optional[list] = None):
        """
        ...
        if codes is not None and languages is not None:
            ...
            tree_features_projected = self.tree_projection(tree_features)  # (batch_size, tree_projection_dim)
        else:
            # Use zero tree features if not provided
            tree_features_projected = torch.zeros(
                batch_size, self.tree_feature_projection_dim, 
        ...
        pooled = [torch.max_pool1d(c, kernel_size=c.size(2)).squeeze(2) for c in conv1_outputs]
...
def load_tokenizer(backbone_type: BackboneType = "codebert", model_name: Optional[str] = None):
    """
...
"""
        ...
        tree_features = model_codebert.extract_tree_features_batch(test_codes, test_languages)
        ...
        print("\nForward pass WITH tree features...")
        with torch.no_grad():
            outputs_with_tree = model_codebert(inputs, codes=test_codes, languages=test_languages)
    ...
    try:
        ...
        train_dataset = dataset_loader.get_dataset(split='train', train_subset=0.001)  # 0.1% for quick test
        ...
        languages = [sample.get('language', 'python').lower() for sample in test_samples]
        ...
        print("Note: You may need to login with 'huggingface-cli login' to access the dataset")
        import traceback
        traceback.print_exc()
    ...
    try:
        ...
        model_codebert.eval()
        with torch.no_grad():
            outputs = model_codebert(inputs, codes=multi_lang_codes, languages=multi_lang_languages)
